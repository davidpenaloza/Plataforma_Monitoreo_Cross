...
Componente Blockgrade (ADF + tabla blockgrade_bybucket).
...
signals
| extend status_global = Blockgrade
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_ingestas_blockgrade; source=statusFuentes_oficial_ada")
