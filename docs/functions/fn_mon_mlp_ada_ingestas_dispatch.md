...
Componente Dispatch (incluye ADF_dispatch_ingestion).
...
signals
| extend status_global = Dispatch
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_ingestas_dispatch; source=statusFuentes_oficial_ada")
