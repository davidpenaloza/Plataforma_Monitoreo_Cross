...
Componente Meteo (jobs meteo y timestamp meteodata).
...
signals
| extend status_global = Meteo
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_ingestas_meteodata; source=statusFuentes_oficial_ada")
