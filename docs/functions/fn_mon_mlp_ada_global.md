...
## Objetivo
Obtener estado de **global** para ADA MLP basado en la query oficial actualizada compartida por el equipo.
...
Usa regla ejecutiva agregada de fuentes ADA oficiales.
...
signals
| extend status_global = iff(Dispatch=="ALERT" or PI=="ALERT" or Drillit=="ALERT" or Blockgrade=="ALERT" or Plans=="ALERT" or Meteo=="ALERT" or Settings=="ALERT", "ALERT", "OK")
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_global; source=statusFuentes_oficial_ada")
