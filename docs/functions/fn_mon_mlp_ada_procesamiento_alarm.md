...
## Objetivo
Obtener estado de **procesamiento** para ADA MLP basado en la query oficial actualizada compartida por el equipo.
...
Componente Alarmas basado en jobs 06/07 e incidentes largos.
...
signals
| extend status_global = Alarmas
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_procesamiento_alarm; source=statusFuentes_oficial_ada")
