...
## Objetivo
Obtener estado de **front** para ADA MLP basado en la query oficial actualizada compartida por el equipo.
...
Componente Front basado en AlertarFront().
...
signals
| extend status_global = Front
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_front; source=statusFuentes_oficial_ada")
