...
Componente Plans basado en job01_plans + tablas planes.
...
signals
| extend status_global = Plans
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_ingestas_plans; source=statusFuentes_oficial_ada")
