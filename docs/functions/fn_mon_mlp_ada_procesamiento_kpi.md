...
Componente KPI basado en statusKpisAlarms del query oficial.
...
signals
| extend status_global = KPIs
| extend status = iff(status_global == "ALERT", "ALERT", "OK")
| extend color = case(status == "ALERT", "#E53935", "#EAF4EA")
| extend evidence = strcat("component=fn_mon_mlp_ada_procesamiento_kpi; source=statusFuentes_oficial_ada")
